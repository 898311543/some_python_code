...
def mail():
    ret=True
    try:
        msg=MIMEText('您的环境检测系统检测到数据异常，请及时查看','plain','utf-8')
        msg['From']=formataddr(["Tim",my_sender])   #括号里的对应发件人邮箱昵称、发件人邮箱账号
        msg['To']=formataddr(["Tim",my_user])   #括号里的对应收件人邮箱昵称、收件人邮箱账号
...
ret=mail()
if ret:
    exit(0) #如果发送成功则会返回ok，稍等20秒左右就可以收到邮件
else:
    exit(1)  #如果发送失败则会返回filed
